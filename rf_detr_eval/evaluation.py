"""Comprehensive Evaluation & Visualization for RF-DETR on BDD100K.
...
Output:
    All plots and metrics are saved to ``<BASE_DIR>/rf_detr_evaluation_output/``.
"""
...
def _plot_attribute_bar(data: dict, title_suffix: str, filename: str):
    ...
    ax.set_ylabel("mAP")
    ax.set_title(f"Performance by {title_suffix} — RF-DETR")
    ax.set_xticks(x)
...
def plot_per_class_ap(per_class_ap: dict):
    ...
    ax.set_title("Per-Class Average Precision — RF-DETR on BDD100K Val")
    ax.set_xticks(x)
...
def compute_confusion_matrix(coco_gt, pred_data, cat_id_to_name,
    ...
    ax.set_title("Confusion Matrix — RF-DETR on BDD100K Val (IoU>=0.5)")
    plt.tight_layout()
...
def plot_performance_by_size(metrics: dict):
    ...
    ax.set_ylabel("Score")
    ax.set_title("Performance by Object Size — RF-DETR")
    ax.set_xticks(x)
...
def plot_precision_recall_curves(coco_gt, coco_dt, cat_id_to_name):
    ...
    plt.suptitle("Precision-Recall Curves (IoU=0.5) — RF-DETR",
                 fontsize=14, y=1.02)
    plt.tight_layout()
...
def plot_f1_vs_confidence(gt_data, pred_data):
    ...
    ax.set_title("F1 Score vs Confidence Threshold — RF-DETR")
...
def failure_analysis(per_class_ap, metrics, gt_data, pred_data, cat_id_to_name):
    ...
    lines += [
        ...
        "  3. Model: try RF-DETR-Large for improved small-object AP",
        "  4. Model: increase input resolution 640 -> 1280",
        "  5. Post-processing: class-specific confidence thresholds",
        "  6. Training: longer training schedule or more augmentations",
    ]
...
def main():
    ...
    print("=" * 60)
    print("RF-DETR COMPREHENSIVE EVALUATION ON BDD100K")
    print("=" * 60)
