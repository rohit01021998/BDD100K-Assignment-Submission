...
Loads COCO ground-truth and YOLO predictions into FiftyOne,
...
def main(
    ...
    for img_id, img_preds in preds_by_img.items():
        ...
        sample["yolo11s"] = fo.Detections(detections=detections)
        sample.save()
        added += 1
    ...
    results = dataset.evaluate_detections(
        "yolo11s", gt_field="detections",
        eval_key="eval", compute_mAP=True,
    )

    print("\n" + "=" * 60)
    print("EVALUATION RESULTS (YOLO11s on BDD100K val)")
    print("=" * 60)
    ...
    print("    python -m yolo11s_eval.fo_launch")
    print("=" * 60 + "\n")
